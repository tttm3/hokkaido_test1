...
const fertilizerData = [
    ...
    },
    {
        ...
        "lines": [
            ...
            "荷姿1：　バラ (2t)",
            "販売価格1：　5,800円/バラ (2t)",
            "散布可能地域：　倶知安町、ニセコ町、京極町",
            "散布可能時期：　周年"
        ]
    },
    {
        "id": 34,
        "areaId": "abashiri-kitami-monbetsu",
        "municipality": "北見市",
        "company": "北見エコファーム",
        "address": "北海道北見市",
        "livestock": "cattle",
...
function filterResults(params) {
    return fertilizerData.filter(item => {
        // 1. 散布方法のフィルタ（未選択・決めていない場合は全表示、それ以外は一致確認）
...
function renderSearchResults(results, params) {
    ...
    results.forEach((item) => {
        ...
        // エリアIDから日本語のエリア名を取得
        const areaName = hokkaidoAreas[item.areaId] ? hokkaidoAreas[item.areaId].name : '北海道';
        // 「○○エリア」と長くなるので、簡易的に表示したい場合は調整してください（ここではそのまま表示）
